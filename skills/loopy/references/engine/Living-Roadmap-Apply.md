...
Supported operation types are `add`, `rename`, `combine`, `split`, `prioritize`, `defer`, `cancel`, `supersede`, `set-dependencies`, `set-blockers`, `record-participation`, `select`, and `reconcile-product`. Use `reconcile-product` only after a newer Product foundation has been separately confirmed; it re-anchors unchanged portfolio state and does not approve or apply a material change. Use the exact fields described by the current Engine types and contract; do not invent workspace, worktree, commitment, or approval fields.
