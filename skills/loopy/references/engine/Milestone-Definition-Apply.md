...
- `definition-revalidate` preserves uncommitted Definition identities and content when the never-confirmed Product draft evolves during the same discovery interview. First reconcile the Living Roadmap to the current Product draft. Then provide an actor with represented perspectives and a concise semantic-revalidation basis. This invalidates prior review and coherence, records history, and keeps the Definition in draft for the next targeted update. It cannot bypass Change Management for a previously confirmed Product boundary or committed Definition.
