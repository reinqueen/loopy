...
Resolve the package root containing `skills/loopy/`, `engine/`, and `templates`. Before constructing an operation input, read [Living Roadmap operation input](../engine/Living-Roadmap-Apply.md). Use the compiled CLI with `--source-root <package-root>` and `--workspace <target-project>`:
